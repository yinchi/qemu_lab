/* AArch64 exception vector table: 16 entries, 0x80 bytes apart, 0x800 total.
   Row = where the exception came from, column = exception type. We only
   expect real work at "Current EL, SPx, IRQ" (offset 0x280); every other
   entry traps to unexpected_exception() so a bug is visible instead of
   silently corrupting execution. */
...
matching the index in the vector table.

For all entries except irq_el1h, branch to unexpected_exception(), where the value in x0
becomes the argument `vector` (see main.c). For irq_el1h, branch to irq_handler() instead.
*/
