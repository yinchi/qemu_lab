...
MALFORMED_ELFS = [
    ...
    "elf-toolargefile.exe",  # a valid program in a file bigger than half the kernel heap
]
...
def run(ctx):
    ...
    check("probe without a subcommand", s.run("tests/probe.exe"),
          "tests/probe.exe\nusage: probe sys-unknown|bad-ptr|fds|close-out|clock|leak-write|reboot-wide|getdents-small|args|exit|poke|poke-w|user-ptrs|ioctl|getcwd|sp|stack|frag|frag-raw|bs-wide|interleave ...\nexit 2\n")
