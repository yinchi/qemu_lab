...
/// Base address of the PL031 real-time clock, which `virt` provides with no `-device` flag (like the
/// UART and the GIC), at a fixed address; hard-coded like `UART0_BASE`. Its first register, `RTCDR`, is a
/// read-only count of seconds since the Unix epoch -- checked to match the host's `date +%s`.
pub const RTC_BASE: usize = 0x0901_0000;
/// Size of the RTC's memory-mapped I/O region (one page; the PL031's registers span 0x1000).
...
/// Start of the EL0-accessible user window in virtual memory. Every program is linked here.
pub const USER_BASE: usize = 0x4400_0000;
/// The window's *ceiling*: the most address space a program can have, image and stack together. What is
/// actually mapped is only what the program being loaded needs (`exec/elf.rs`), so a small program still
/// costs a few pages; the rest of the window stays unmapped, and reachable by no one.
///
/// Sized to fit the RAM QEMU's `virt` gives by default (128 MiB from 0x4000_0000): the kernel image and
/// its 16 MiB heap sit below `USER_BASE`, the window ends at 0x4600_0000, and the top 32 MiB is left free.
pub const USER_SIZE: usize = 0x0200_0000; // 32 MiB
...
//   |  image (up to ~31 MiB)  ...  |  guard, unmapped  |        stack, 1 MiB           |
//                                       (64 KiB)
//
// Only the pages the program's segments and stack need are mapped; everything else in the window --
// the gap after the last segment (where a heap will grow, Stage 16), the guard -- is unmapped, so a
// stack that overflows, or a wild pointer into the gap, faults instead of silently running into the
// program's own data. The stack stays at the top whatever the image's size.
