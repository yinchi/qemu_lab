...
use crate::base_addresses::UART0_BASE;
...
/// The reserved virtio-mmio window: 32 slots of 0x200 bytes each, starting
/// at 0x0A000000 -- a fixed property of the QEMU `virt` machine, documented
/// in `docs/memory_regions.md`, not something derived from which slots
/// happen to be populated. Mapping the whole window (rather than only the
/// slots `base_addresses::init_base_addresses` found an actual device in)
/// is harmless: nothing reads an unpopulated slot's offsets, since
/// `Blk::find`/`Gpu::find` only ever touch slots they've confirmed.
const VIRTIO_MMIO_BASE: usize = 0x0A00_0000;
const VIRTIO_MMIO_SIZE: usize = 32 * 0x200;

/// Fixed EL0-accessible user window -- deliberately well away from the
/// kernel's own region (separated by a 48 MiB unmapped gap), on the same
/// reasoning as leaving the rest of RAM unmapped below: an off-by-one in
/// this module's own boundary math lands in genuinely unmapped memory and
/// faults immediately, rather than landing ambiguously on an adjacent
/// mapped region with the wrong permissions.
