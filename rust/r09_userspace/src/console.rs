...
use crate::static_mut_opt;
use crate::uart::UartWriter;

/// Console foreground color.
pub const DEFAULT_FG: u32 = 0xFF55FF55;
...
/// Writes formatted text to the console (both the GPU text console and the UART, for dual
/// visibility), then flushes the GPU framebuffer to the display -- unlike real VGA VRAM,
/// virtio-gpu writes aren't automatically visible on screen without this. The canonical way
/// anything in this kernel prints text: a user program's `write` syscall (`fd.rs`), a kernel-side
/// diagnostic (the segfault path in `syscall.rs`), and the panic handler all go through this
/// rather than each writing to the console/UART/GPU statics separately.
///
/// Does not handle any control sequences like ANSI escape codes, beyond \n, \r, and \t; however,
/// these may be handled by the host terminal the UART is connected to.
///
/// Safe to use before `CONSOLE`/`GPU` are populated -- the UART write always happens
/// unconditionally, while the console/GPU steps are skipped silently if either isn't there yet,
/// rather than panicking. This matters because the panic handler is reachable from any point in
/// `kernel_main`, including well before `CONSOLE`/`GPU` exist (`Blk::find`, `FatVolume::open`, and
/// every other early `.expect()` can all panic first) -- a panic handler that itself panics
/// trying to reach an uninitialized console would silently swallow the very diagnostic it exists
/// to report.
pub struct ConsoleWriter;

impl core::fmt::Write for ConsoleWriter {
    fn write_str(&mut self, s: &str) -> core::fmt::Result {
        // SAFETY: this project runs single-threaded, so there is never a concurrent access to
        // guard against -- see this struct's own doc comment for why `None` is handled, not just
        // asserted away.
        if let Some(console) = unsafe { static_mut_opt!(CONSOLE) } {
            for c in s.chars() {
                console.write_char(c, DEFAULT_FG, DEFAULT_BG);
            }
        }
        let mut uart0_writer = UartWriter {
            uart: &crate::UART0,
        };
        uart0_writer.write_str(s)?;
        if let Some(gpu) = unsafe { static_mut_opt!(GPU) } {
            gpu.flush();
        }
        Ok(())
    }
