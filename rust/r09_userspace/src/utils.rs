...
//! currently just the three macros every `static mut Option<T>` in this crate (`devices.rs`'s
//! `IDMAP`/`BLK`/`GPU`/`CONSOLE`) is reached through.
...
/// each static this is used on documents why that holds where it's declared (`devices.rs`; 
/// `r07_keydetect`'s `keymap.rs` -- omitted in this crate as the keyboard is not used).
#[macro_export]
...
/// Expands to `Option<&'static mut T>`, given the bare name of a `static mut Option<T>` -- the
/// non-panicking counterpart to `static_mut_ref!`, for a caller that can do something sensible
/// when the static isn't populated yet instead of treating that as a bug (e.g. `ConsoleWriter`,
/// reachable from a panic that can happen before `CONSOLE`/`GPU` exist).
///
/// Deliberately does *not* wrap its own expansion in `unsafe {}`, same as `static_mut_ref!`.
///
/// SAFETY (every call site): same as `static_mut_ref!`.
#[macro_export]
macro_rules! static_mut_opt {
    ($name:ident) => {
        (*(&raw mut $name)).as_mut()
    };
}
