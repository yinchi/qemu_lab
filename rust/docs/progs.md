...
| `poweroff` | 12 | `poweroff` (`poweroff(8)`; not POSIX) | powers off the machine via PSCI `SYSTEM_OFF`, reached through the kernel's `reboot` syscall; `--reboot` restarts it instead (PSCI `SYSTEM_RESET`) | scheduling (`shutdown`'s mandatory `TIME` operand) -- there is no RTC until Stage 13, so every action is immediate; `-f`/`-n`/`-w`/wall messages |
