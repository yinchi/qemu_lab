...
| `date` | 13 | `date [-u] [-d @SECONDS] [-I[FMT] \| -R \| +FORMAT]` | prints the time from the real-time clock, in UTC (the only zone: `%Z` is `UTC`, `%z` is `+0000`), in GNU's default layout (`Sun Sep  9 01:46:40 UTC 2001`); `+FORMAT` takes `chrono`'s `strftime` conversions (`%Y %m %d %H %M %S %s %a %A %b %B %e %j %F %T %D %R %p %u %w %U %W %G %V ...`, see its documentation) with the flags `-` (no padding), `_` (space) and `0`; a conversion it does not know is an error, `date: invalid format '+...'`, where GNU would print it as written; `-I[date\|hours\|minutes\|seconds]` (ISO 8601) and `-R` (RFC 5322); `-d @SECONDS` prints that instant instead of now (negative allowed); `-u` is accepted and changes nothing | setting the time (`-s`, or a `MMDDhhmm` operand) and reading it from a string other than `@SECONDS` (`-d tomorrow`); time zones and `$TZ` until Stage 17; the `^` flag and locale names; sub-second output (`%N`, `-Ins`): the PL031 counts whole seconds; the clock's own range is 1970 to 2106 |
...
Tests for these programs live in the stage that builds them: in `r14_file_times/test/cases/`, `core_utils.py` (the Stage
