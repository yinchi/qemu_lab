...
The kernel is bare-metal AArch64 `no_std` code, so it cannot run `cargo test` on itself. `r12_shell` is
tested in two layers, both run by `just test` (in `rust/r12_shell/`), which first runs a third, cheaper check that the
...
`rust/r12_shell/hosttests/` is a separate small Cargo crate (its own workspace, so it does not inherit
...
| Area | Modules |
|---|---|
| Shell | `lexer`, `syntax`, `frame_stack`, `path` |
...
Everything test-only lives under `rust/r12_shell/test/` or `disk/tests/`, never in `user/`, which holds only
...
```console
> cd rust/r12_shell && just build-test disk
> python3 -c "
import sys, os
sys.path.insert(0, 'test'); os.chdir('test')
from run_tests import run_group
from cases import line_editing
for name, got, want in run_group(os.path.abspath('../r12_shell-test.elf'),
