...
The kernel is bare-metal AArch64 `no_std` code, so it cannot run `cargo test` on itself. The current stage (`<stage>`) is
tested in two layers, both run by `just test` (in `rust/<stage>/`), which first runs a third, cheaper check that the
...
`rust/<stage>/hosttests/` is a separate small Cargo crate (its own workspace, so it does not inherit
...
Groups run in a thread pool, at most `cpus - 1` at a time (`QEMU_TEST_WORKERS=N` overrides that: worth setting when other work is using the machine). A module that sets `EXCLUSIVE = True` (today `token_queue`, whose "typing during a large copy" check races real key presses against a 3 MiB copy and can lose a key when other sessions starve the machine) is left out of the pool and run **by itself** once every other group has finished, so the rest keep the full parallelism. Each group's PASS/FAIL lines are collected and
...
Everything test-only lives under `rust/<stage>/test/` or `disk/tests/`, never in `user/`, which holds only
...
| `disk/tests/` | On the image as `/tests/`: checked-in text/binary fixtures, plus the generated test programs (`probe`, `spin`, ...) and malformed ELFs (`elf-*`) |
...
| `fstab`, `fstab_nodisk`, `fstab_bad`, `fstab_missing` | Stage 19's `/etc/fstab`, read by the shell at start-up before the environment: `HOME` mounted on `/root` and its own `.profile` run (so the mount came first), another volume with `defaults,fail`, a `noauto` line left for `mount` by hand, `nofail` silent for a missing volume, and lines refused for no matching volume, an already-mounted volume, a mount point in use and one that does not exist -- each one serial note and the count (`fstab`); the image's own line with no home disk attached, the shell starting in an empty `/root` on the system volume (`fstab_nodisk`); every kind of bad line (source, mount point, type, `ro`, unknown option, too few and too many fields) with its line number, a `/` line for another volume noted and one for the root silent, and `..` and a trailing slash normalized (`fstab_bad`); no file at all (`fstab_missing`). A module supplies `FSTAB` (text, or `None` for no file) the way it supplies `ENVIRONMENT`, an empty file by default so that a group's disks are not mounted by the image's own line; an extra disk description may have `files` (`{"/name": text}`) copied in |
| `mounts` | Stage 19's mounting, by hand: `mount` listing; `mount LABEL=`/`UUID=` on a directory (a duplicated label picks the first volume by device order, the ID picks the other), the volume's contents replacing and hiding what was in the directory and `umount` uncovering it; paths through a mount (`cd` in and out, `..` out of it, relative paths from inside, `find /`); files created, appended, copied (`cp -r` a tree), moved and removed on a mounted volume, and a program copied there, `chmod +x`'d and run from it; a mount inside a mount; a rename across two volumes refused (`Invalid cross-device link`, for a file and for a directory) with nothing lost; a mount point that cannot be removed, renamed or mounted over; every `mount` and `umount` failure and usage error; the busy rules for `umount` (another mount inside, the working directory inside); and, from the host afterwards, that what was written through each mount is on its own disk and all three extra images pass `fsck.fat` |
| `disks`, `disks_first`, `disks_odd`, `disks_many` | Stage 19's block devices: a second disk found and named by its boot sector (label and volume ID) on the serial log and through `lsblk` (`blkinfo`), with the system image (labelled `SYSTEM` by `just disk`) as the root wherever QEMU puts it (`disks` attaches the extra disk after the system image on the command line, `disks_first` before it); disks that are not FAT (blank, random bytes) listed and ignored, with the root then the first FAT volume, not device 0 (`disks_odd`); more disks than the kernel drives, one left alone with a note, and volumes sharing a label told apart by their IDs (`disks_many`); and that nothing is written to an extra disk (byte-for-byte unchanged afterwards). A module attaches disks with `EXTRA_DISKS` (a list of descriptions -- `{"label", "volume_id", "size_kib"}` for a FAT disk, `{"kind": "blank"}` or `{"kind": "noise"}` -- each with `"before": True` to go before the system image on the command line) and can relabel the system image with `SYSTEM_LABEL` and `SYSTEM_ID` (`disks_odd` sets the old label `R12SH`); the context's `extra_imgs` and `extra_hashes` are for `verify_disk` |
| `textflags` | Stage 18's text-tool flags: `cat -n -E -T -s` (numbering across files, squeezing, the unterminated last line, binary safety), `head`/`tail` with several files (headers, `-q`, `-v`, a failing file among them), `head -n -N`/`-c -N` and `tail -n +N`/`-c +N` (files, stdin, a 100000-line stream, every bad count), `wc -m` (against the CJK fixture, in GNU's column order) and `echo -e -E` (every escape, `\c`, grouped and separate options, what is not an option) |
| `filters` | Stage 18's filters, as whole programs through files and pipes: `sort` (text and `-n` order, `-r`, `-u`, several files, stdin, no trailing newline, a 20000-line run), `uniq` (`-c -d -u`), `cut` (`-f` with every list form, `-s`, `-c` on multi-byte characters, the default tab through `tr`, every error), `tr` (translate, pad, `-d`, `-s`, classes, escapes, UTF-8 passing through), `find` (order, `-name`, `-type`, depth limits, several paths, `.` as the default) and `fgrep` (fixed strings, `-i -v -n -c -l -q -e`, several files, stdin, the three statuses). The pure parts are host tests |
| `flags` | Stage 18's flag catch-up, and the changes to `ls`'s defaults: `mkdir -p -v`, `cp -r -n -v` (trees, merging, into itself, the file in the way), `mv -n -v -f`, `rm -v -d`, and `ls -a -d -R -r -S -t -h` (dot-names hidden, a file operand listed as itself, headers, `-t` with real waits between the files it sorts, `-h` against an independent formula), plus the `--help` text of the five |
| `tools` | Stage 18's small programs: `touch` (that it moves an existing file's modify time forward, after a real-time wait for the 2-second FAT tick, and changes nothing else -- size, creation time, data, exec bit -- plus creation, `-c`, several operands, a directory, a read-only file), `rmdir`, `seq` (every form, `-s`, `-w`, negative numbers, the end of the i64 range, a 100000-line run) and `cmp` (a difference with its byte and line, prefix EOF both ways, `-s`, stdin, a 3 MiB file, every error) |
| `profile`, `profile_bad`, `profile_big` | `$HOME/.profile`: run before the first prompt in the shell itself (an `export`, a plain assignment, `PATH=$PATH:...`, a `PS1`, a command's output, a `cd`, and failing lines with more after them: the rest still run), `$?` at the first prompt is the profile's last command's; a profile that is not UTF-8 or is over 64 KiB is one serial note and skipped. A module supplies `PROFILE` (text or bytes) the way it supplies `ENVIRONMENT`; the harness writes it to `$HOME/.profile` in the group's image copy. A missing profile is silent (`home` checks the log) |
| `prompt`, `prompt_env` | `$PS1`: `\w`, `\W`, `\$`, `\\` and unknown escapes at the next prompt (a `cd` shows at once), no `$` expansion, empty and unset are the default, not exported until asked, a stage of a pipeline does not change it, editing and history after a longer prompt, a prompt that wraps the row (a screendump of an edited line against the same text typed in one go), and `PS1` in `/etc/environment` at the very first prompt, with a wide character in it. A transcript ends at the prompt's final `> `, so a longer prompt leaves its front part on the end of what `run` returns |
| `path` | `$PATH`: a bare name searched in order through a list of directories (exactly as typed, in each) with two directories of test programs under `/tmp`: a prefix for one command, `PATH=$PATH:...`, precedence, `source`/`.` finding a script through `PATH` before the working directory (and the working-directory fallback, a slash name not searched, a directory of that name skipped, `sh` cwd-only), exact names (a file called `dotted.exe` is `dotted.exe`, and `cat.exe` finds nothing), a directory or a file without the exec bit skipped or refused, unset (`/bin`), empty (nowhere) and empty entries (skipped), relative entries, and that a name with a `/` ignores `PATH`. The candidate list is a host test |
