...
| 35 | `unlinkat` (`unlink`) | `path, path_len, flags` | `0`. Removes a file, or with `flags & AT_REMOVEDIR` (`0x200`) an empty directory. The path is normalized first, so `.` and `..` mean the current and parent directories. `EISDIR`, `ENOTDIR`, `ENOTEMPTY`; `EINVAL` for a path that is `/`, or for flags other than 0 and `AT_REMOVEDIR`. **From Stage 19:** `EBUSY` for a mount point. |
| 38 | `renameat` (`rename`) | `old, old_len, new, new_len` | `0`. Refuses an existing destination (`EEXIST`) and moving a directory into itself (`EINVAL`). **From Stage 19:** `EXDEV` if the two paths are on different volumes (nothing is copied: a program copies and removes), and `EBUSY` if either is a mount point. |
...
| 40 | `mount` | `source, source_len, target, target_len` | `0`. Mounts the volume `source` names on the directory `target` (a path from the working directory like any other). `source` is `LABEL=name` (the volume label, compared without regard to ASCII case; if two volumes share it, the first by device order) or `UUID=XXXX-XXXX` (the FAT volume ID, hex either case): the spellings of an `fstab` line. Errors: `EINVAL` (neither spelling, or not a filesystem the kernel can open), `ENODEV` (no device matches), `EBUSY` (that volume is mounted already -- a volume is mounted once -- or `target` is a mount point, or another mount is inside `target`, which the new mount would cover), `ENOENT`/`ENOTDIR` (the target is not a directory), `ENAMETOOLONG` (over 256 bytes). What was in the directory is hidden until the volume is unmounted. Linux's number, much simplified shape: no filesystem type, flags or data. **From Stage 19.** |
| 39 | `umount2` (`umount`) | `target, target_len` | `0`. Unmounts the volume mounted exactly at `target`. `EINVAL` if nothing is mounted there; `EBUSY` for the root, for a mount with another mounted inside it, and for a volume with an open file on it or a working directory of the shell (any frame's) inside it. Linux's number, without its flags. **From Stage 19.** |
| 1000 | `blkinfo` | `index, out` | `0`, after writing a 288-byte record (`abi::blk`, `BLKINFO_SIZE`) describing block device `index` to `out`: `capacity: u64` in bytes, `flags: u32` (`BLK_FAT`: the device's first sector is a FAT boot sector, so the next two fields mean something; `BLK_ROOT`: it is the root of the file tree), `volume_id: u32` (the FAT volume ID), `label_len: u8` and the 11-byte `label`, `mount_len: u16` and, in the last 256 bytes, the `mount` point (empty, `mount_len` 0, for a device that is not mounted), little-endian. Devices are numbered from 0 in the order the kernel found them. `ENODEV` for an index past the last device (so a caller counts them by asking until it gets that), `EFAULT` for a bad `out`. **From Stage 19. Not a Linux syscall** (Linux reports this through `/sys` and device nodes, which this kernel has neither of), so its number, 1000, lies outside the range Linux uses. `lsblk` is the program over it. |
...
| `EBUSY` | -16 | Device or resource busy (`mount`/`umount` refusals; `unlink`/`rename` of a mount point; from Stage 19) |
| `EEXIST` | -17 | File exists |
| `EXDEV` | -18 | Invalid cross-device link (`renameat` between two volumes; from Stage 19) |
| `ENODEV` | -19 | No such device (`blkinfo` past the last device, `mount` of a source no device matches; from Stage 19) |
