...
carries; the source is `rust/r15_large_binaries/src/shell/` (plus `exec/frame_stack.rs` and `exec/shell_state.rs`
for its state).
...
than in QEMU (`rust/r15_large_binaries/hosttests/`, run by `just test-host`):
