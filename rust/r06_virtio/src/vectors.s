/* AArch64 exception vector table: 16 entries, 0x80 bytes apart, 0x800 total.
   Row = where the exception came from, column = exception type. This stage
   never unmasks IRQs (no interrupt-driven I/O -- everything is polled), so
   every entry, `irq_el1h` included, is expected to trap to
   unexpected_exception() -- a bug is visible instead of silently corrupting
...
value in x0 becomes the argument `vector` -- this stage has no legitimate exception source at all.
*/
