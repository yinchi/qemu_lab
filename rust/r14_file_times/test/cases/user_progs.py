...
BUILD_STAMP = datetime.fromtimestamp(1767225600, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")  # UTC, as the image is built
...
def run(ctx):
    ...
    def stat_of(name):
        """`stat`'s three timestamps for `name`: (modify, create, access-date), and the rest of the block."""
        out = s.run(f"stat {name}").split("\n")
        fields = {line.split(":", 1)[0].strip(): line.split(":", 1)[1].strip() for line in out[1:] if ":" in line}
        return fields["Modify"], fields["Create"], fields["Access"], fields
    ...
    modify, create, access, fields = stat_of("tests/statdir")
    ...
    check("...and its access date is today (UTC)",
          access in {time.strftime("%Y-%m-%d", time.gmtime(t)) for t in (before, after)}, True)

    modify, create, access, fields = stat_of("tests/statfresh.txt")
    ...
    after = time.time()
    modify2, create2, _access, _fields = stat_of("tests/statfresh.txt")
    ...
    after = time.time()
    modify3, _create3, _access, _fields = stat_of("tests/statfresh.txt")
    check("rewriting a file stamps its modify time", recent(modify3, before, after), True)

    before = time.time()
    s.run("echo made > tests/statredir.txt")
    after = time.time()
    modify4, create4, _access, _fields = stat_of("tests/statredir.txt")
    ...
    check("a bundled file still has its build-time stamp", stat_of("tests/hello.txt")[:2], (BUILD_STAMP, BUILD_STAMP))
