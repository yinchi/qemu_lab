#!/usr/bin/env python3
"""Boots r14_file_times headless and runs every test case module in `cases/` against it, then checks what
...
from cases import core_utils, launch, console, unicode, stack, line_discipline, wrapped_input, token_queue, cwd, syntax, redirection, scripts, user_progs, pipes, power, line_editing, stack_guard, clock
...
def run_group(elf, orig_img, disk_dir, modules):
    ...
    workdir = tempfile.mkdtemp(prefix=f"r14-{label}-")
