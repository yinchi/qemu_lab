...
//! and a fresh stack into the fixed user window `arch/mmu.rs` reserves, mapping every page with its
...
//!
//! The window is the same fixed range for every program, so each load starts by unmapping all of
//! what the previous program left. The pages a segment lands on are then mapped *writable* just long
//! enough to fill (the kernel's own writes obey the page permissions too, now that the MMU is on),
//! and locked down to the segment's real permissions afterwards. What ends up mapped, and how, is
//! recorded in `usermem` so a syscall can check a user pointer before touching it.
...
pub fn load(elf_bytes: &[u8]) -> Result<usize, ElfError> {
    ...
    // 1. Nothing of the previous program stays mapped.
    set_pages(USER_BASE, USER_BASE + USER_SIZE, None)?;
