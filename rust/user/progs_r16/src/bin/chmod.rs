...
use userlib::{ATTR_DIRECTORY, ATTR_EXEC, ATTR_READ_ONLY, ExitCode, chmod, stat};
...
/// subdirectories alike -- `path` itself is not touched (the caller already did that). Each directory is listed
/// whole and its descriptor closed *before* recursing, so the depth of the tree is not limited by how many files
/// the kernel lets a program hold open (Stages 12-15 kept every ancestor's directory open while descending, which
/// gave out around ten levels down). Returns the first error, after attempting everything.
fn chmod_recursive(path: &str, set: u8, clear: u8) -> Result<(), isize> {
    let entries = read_dir(path).map_err(|e| e.errno())?;
    let mut first_err: Option<isize> = None;
    for ent in entries {
        let child = join(path, &ent.name);
        if ent.attrs & ATTR_DIRECTORY != 0
            && let Err(e) = chmod_recursive(&child, set, clear)
        {
            first_err.get_or_insert(e);
        }
        let r = chmod(&child, set, clear);
        if r < 0 {
