...
//! zone (`$TZ`, UTC if unset or unknown: `progs_r17::local_zone`) for display, with the zone's abbreviation, as
//! `date` shows it: `2001-09-08 21:46:40 EDT`.
//!
...
fn write_stamp(out: &mut Fd, zone: Tz, label: &str, date: u16, time: u16) {
    ...
    let local = utc.and_utc().with_timezone(&zone);
    let _ = writeln!(
        out,
...
fn print_one(name: &str, zone: Tz, info: &Stat) {
    let mut out = Fd(1);
    ...
    write_stamp(&mut out, zone, "Modify", info.modified_date, info.modified_time);
    write_stamp(&mut out, zone, "Create", info.created_date, info.created_time);
}
...
fn run(args: userlib::Args) -> ExitCode {
    ...
    for (i, arg) in cli::operands(args).enumerate() {
        ...
        match stat(arg) {
            Ok(info) => print_one(arg, zone, &info),
            Err(e) => {
