...
//! real-time clock (or the time `-d @SECONDS` names) in the local time zone -- named by `$TZ`, UTC if it is
//! unset or not a zone the database knows (`progs_r17::local_zone`) -- or in UTC with `-u`.
//!
...
//! this binary, which is why Stage 15's larger program window exists. `chrono` formats through `alloc`, which is
//! the user heap (`userlib`'s `heap` feature, Stage 16), grown on demand: `date` no longer carries a heap of its own.
