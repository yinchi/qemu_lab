...
//! real-time clock (or the time `-d @SECONDS` names) in UTC, the only zone there is.
//!
//! The calendar arithmetic and the `strftime` conversions are `chrono`'s.
...
fn parse(args: userlib::Args) -> Result<Request, ExitCode> {
    ...
    while let Some(arg) = cli::next("date", &mut opts)? {
        let chosen = match arg {
            ...
            // Every time is UTC already.
            Arg::Short('u') | Arg::Long("utc" | "universal") => continue,
    ...
    Ok(Request { format: format.unwrap_or(DEFAULT_FORMAT), at })
}
...
/// The layout for `-I[FMT]`.
fn iso_format(precision: Option<&str>) -> Result<&'static str, ExitCode> {
    match precision {
        None | Some("date") => Ok("%Y-%m-%d"),
        Some("hours") => Ok("%Y-%m-%dT%H+00:00"),
        Some("minutes") => Ok("%Y-%m-%dT%H:%M+00:00"),
        Some("seconds") => Ok("%Y-%m-%dT%H:%M:%S+00:00"),
        Some(other) => {
...
fn run(args: userlib::Args) -> ExitCode {
    ...
    // A conversion `chrono` does not know makes its `Display` fail; nothing has been written by then.
    if write!(Fd(1), "{}", instant.format(request.format)).is_err() {
        let _ = writeln!(Fd(2), "date: invalid format '+{}'", request.format);
        return ExitCode(1);
    }
    let _ = writeln!(Fd(1));
    ExitCode(0)
