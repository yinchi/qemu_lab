//! `wc [-l] [-w] [-m] [-c] [-L] [file...]` -- see `docs/progs.md`. Stage 18's tier adds `-m` (count characters: UTF-8 characters,
//! so a multi-byte one is one) to the Stage 12 `wc`. Counts print in GNU's order: lines, words, characters, bytes, longest line.
...
const USAGE: &str = "wc [-l] [-w] [-m] [-c] [-L] [file...]";
const FLAGS: &[(&str, &str)] = &[
    ("-l", "count lines"),
    ("-w", "count words"),
    ("-m", "count characters"),
...
impl Counts {
    fn add(&mut self, chunk: &[u8]) {
        ...
        // A character is a byte that is not a UTF-8 continuation byte (`10xxxxxx`).
        self.chars += chunk.iter().filter(|&&b| b & 0xC0 != 0x80).count();
        for &b in chunk {
            if b == b'\n' {
                self.lines += 1;
...
fn print_counts(c: &Counts, wanted: [bool; 5], name: Option<&str>) {
    let mut out = Fd(1);
    let mut first = true;
    for (wanted, count) in wanted.into_iter().zip([c.lines, c.words, c.chars, c.bytes, c.max_line]) {
        if wanted {
...
fn count(args: userlib::Args) -> Result<ExitCode, ExitCode> {
    let (mut lines, mut words, mut chars, mut bytes, mut max_line) = (false, false, false, false, false);
    ...
    if !(lines || words || chars || bytes || max_line) {
        (lines, words, bytes) = (true, true, true);
    }

    let wanted = [lines, words, chars, bytes, max_line];
    let mut status = 0;

    if n_files == 0 {
        match count_fd(0) {
            Ok(c) => print_counts(&c, wanted, None),
            Err(e) => {
                fail("wc", "stdin", e);
                status = 1;
            }
    ...
    for arg in cli::operands(args) {
        ...
        match count_fd(fd) {
            Ok(c) => {
                print_counts(&c, wanted, Some(arg));
                total.accumulate(&c);
            }
            Err(e) => {
                fail("wc", arg, e);
    ...
    if n_files > 1 {
        print_counts(&total, wanted, Some("total"));
    }
