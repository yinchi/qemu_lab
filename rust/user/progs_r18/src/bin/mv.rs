//! `mv [-f] [-n] [-v] SRC... DST` -- see `docs/progs.md`. Stage 18's tier adds `-n` (do not replace an existing target),
//! `-v` (say what was moved) and `-f` (accepted: nothing here ever asks) to the Stage 16 `mv`.
//!
//! `hadris-fat`'s `rename` refuses if `DST` already names something, whatever type it is, so the POSIX `mv`(1) behaviors
//! on top of it -- moving into an existing directory, replacing an existing file -- are entirely this program's own
//! layer: probe `DST` with `stat` first, then decide.
...
/// The move itself, once `target` is settled: probes `target` with `stat`, then renames (replacing a plain-file target
/// first).
