...
/// `chrono` is built with its `alloc` feature (this package's `date` formats with it), so every program here
/// links the `alloc` crate and needs a global allocator, though this one never allocates. A token heap does.
#[global_allocator]
...
fn run(args: userlib::Args) -> ExitCode {
    // SAFETY: `HEAP_SPACE` is used only here, once.
