...
//! currently just the pair of macros every `static mut Option<T>` in this crate (`devices.rs`'s
//! `BLK`/`GPU`/`CONSOLE`/`KEYBOARD`/`IDMAP`, `keymap.rs`'s `KEY_STATE`/`LOCK_STATE`, `main.rs`'s
//! `LINE`/`VOL`) is reached through.
