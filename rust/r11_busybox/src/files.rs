...
//! `fd.rs` owns the small fd numbers a program sees; this module owns what they refer to.
//!
...
use crate::errno::{EACCES, EBADF, EINVAL, EIO, EISDIR, EMFILE, ENOENT, ENOTDIR};
use crate::fat_io::BlkIo;
use crate::{EXEC_BIT, VOL, find_entry, static_ref};

/// FAT's own read-only attribute bit -- what `chmod -w`/`+w` toggles.
const READ_ONLY_BIT: u8 = 0x01;
/// The volume-label pseudo-entry's attribute bit; not a real file, so never listed.
const VOLUME_LABEL_BIT: u8 = 0x08;

/// The only attribute bits `chmod` may change: the two this project exposes as permissions.
const CHMOD_BITS: u8 = EXEC_BIT | READ_ONLY_BIT;

/// Longest file name `getdents` reports (FAT's long-filename limit); anything longer can't exist
/// on this filesystem anyway.
const NAME_MAX: usize = 255;
/// Size of one `getdents` record: `size: u32` (little-endian), `attrs: u8`, `name_len: u8`, then
/// `NAME_MAX` bytes of name, NUL-padded. `userlib`'s `DirEnt` decodes the same layout.
const DIRENT_SIZE: usize = 4 + 1 + 1 + NAME_MAX;

/// How many files/directories may be open at once, across every fd a program holds.
...
fn walk(dirs: &[&str]) -> Result<Dir, isize> {
    ...
    for name in dirs {
        let entry = find_entry(&dir, name).ok_or(ENOENT)?;
...
fn list(dir: &Dir) -> Result<Vec<DirRec>, isize> {
    ...
    for item in dir.entries() {
        ...
        if name == "." || name == ".." || attrs & VOLUME_LABEL_BIT != 0 {
            continue;
        }
        recs.push(DirRec {
...
fn open_read(path: &str) -> Result<OpenFile, isize> {
    ...
    let entry = find_entry(&parent, leaf).ok_or(ENOENT)?;
    if entry.is_directory() {
...
fn open_write(path: &str) -> Result<OpenFile, isize> {
    ...
    let parent = walk(parents)?;
    let entry = match find_entry(&parent, leaf) {
        Some(entry) => {
            if entry.is_directory() {
                return Err(EISDIR);
            }
            if entry.attributes().bits() & READ_ONLY_BIT != 0 {
                return Err(EACCES);
            }
            entry
        }
...
pub fn chmod(path: &str, set: u8, clear: u8) -> isize {
    ...
    let entry = match walk(parents).and_then(|parent| find_entry(&parent, leaf).ok_or(ENOENT)) {
        Ok(entry) => entry,
        Err(e) => return e,
    };
