    ...
    match ec {
        ...
        EC_SVC64 => {
            ...
            match nr {
                ...
                SYS_OPEN => regs.x[0] = crate::fd::open(a0, a1, a2) as u64,
                SYS_CLOSE => regs.x[0] = crate::fd::close(a0) as u64,
                SYS_GETDENTS => regs.x[0] = crate::fd::getdents(a0, a1, a2) as u64,
                SYS_CHMOD => regs.x[0] = crate::fd::chmod(a0, a1, a2, a3) as u64,
                SYS_EXIT => {
                    crate::process::set_exit_code(a0 as i32);
        ...
        EC_IABT_LOWER | EC_DABT_LOWER => {
            ...
            // goes through Console and keeps INPUT_ROW in sync -- see fd.rs), a segfault is an
            ...
            // for the same reason.
            crate::fd::uart_ensure_newline();
            crate::fd::uart_write(
                alloc::format!("Segmentation fault (address {far:#x}, ESR_EL1 {esr:#x})\n")
                    .as_bytes(),
            );
            crate::process::set_exit_code(crate::process::EXIT_FAULT);
