...
//! through the same `input::token_for` -> `LineBuffer::feed` path the prompt uses, so Backspace
...
use crate::devices::{CONSOLE, GPU, KEYBOARD};
use crate::line::LineEvent;
use crate::tokens::KEY_D;
use crate::{BG, FG, LINE, input, show_row, static_mut_ref};
...
fn read_line() -> Option<Vec<u8>> {
    ...
    // are the same statics it uses, with the same at-most-one-user guarantee (see devices.rs).
        ...
        while let Some(event) = kb.poll() {
            ...
            match line.feed(token) {
                ...
                Some(LineEvent::Finished(text)) => {
                    console.write_char('\n', FG, BG);
                    gpu.flush();
                    crate::fd::uart_write(text.as_bytes());
                    crate::fd::uart_write(b"\n");
