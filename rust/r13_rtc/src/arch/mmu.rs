...
use crate::platform::base_addresses::{
    GICC_SIZE, GICD_SIZE, RTC_BASE, RTC_SIZE, UART0_BASE, UART0_SIZE, VIRTIO_MMIO_BASE,
    VIRTIO_MMIO_SIZE,
};
...
pub fn enable(gicd: usize, gicc: usize) -> Hardening {
    ...
    // 1. MMIO: GIC distributor+CPU interface, UART, the RTC, the virtio-mmio window.
    idmap
    ...
    idmap
        ...
        .unwrap();
    idmap
        .map_range(&MemoryRegion::new(RTC_BASE, RTC_BASE + RTC_SIZE), device)
        .unwrap();
    idmap
        .map_range(
