...
def run(ctx):
    ...
    s.run("chmod +x tests/probe")  # self-sufficient: this module must not depend on an earlier
    ...
    check("a bare command name is found in /bin from anywhere", (s.run("cd /tests"), s.run("echo hi")),
          ("cd /tests\n", "echo hi\nhi\n"))
    check("a bare name is not looked up in the working directory", s.run("probe"),
    ...
    check("a program is launched by relative path", s.run("./probe ioctl 3 1"),
          "./probe ioctl 3 1\nioctl(3, 1): -9\n")
    check("...and by a path with ..", s.run("../tests/probe ioctl 3 1"),
    ...
    check("getcwd with room", s.run("./probe getcwd 100"), "./probe getcwd 100\ngetcwd(100): 6 /tests\n")
    check("getcwd with exactly enough", s.run("./probe getcwd 6"), "./probe getcwd 6\ngetcwd(6): 6 /tests\n")
    check("getcwd with too little is ERANGE", s.run("./probe getcwd 5"), "./probe getcwd 5\ngetcwd(5): -34\n")
    check("getcwd with nothing is ERANGE", s.run("./probe getcwd 0"), "./probe getcwd 0\ngetcwd(0): -34\n")
