...
//! of `Gpu::framebuffer()` (see `gpu.rs`, `console.rs`). `IdMap` is `mmu.rs`'s page table, written
//! once there and read again by `elf.rs` on every program load.
...
// `process::run_program` masks every DAIF bit for its entire time there (see `process.rs`'s doc
