...
//!
//! Built on `shlex` (no_std via `default-features = false`, needing only `alloc` + a global
//! allocator, both already set up in `main.rs`) rather than hand-rolling shell-word parsing:
//! quoting/escaping has enough real edge cases (nested quotes, trailing backslashes, ...) that
//! reusing a well-tested implementation is the right call for a kernel-side launcher, not
//! reinventing it.
