...
def run(ctx):
    ...
    s.type("cp tests/bigpad.exe tests/bigcopy.bin\n")
    s.type("echo intact\n")
    check("typing during a large copy loses nothing", wait_for_end(s, "intact\nintact\n> "),
          "cp tests/bigpad.exe tests/bigcopy.bin\n> echo intact\nintact\n")
...
def verify_disk(ctx):
    ...
    ctx.check("disk: the copy made while typing is byte-identical", 
              filecmp.cmp(os.path.join(out, "bigcopy.bin"), ctx.fixture_path("bigpad.exe"), shallow=False), True)
